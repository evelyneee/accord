    ...
    var body: some View {
        List($mentions, id: \.id) { $message in
            ZStack(alignment: .topTrailing) {
                ...
                Button("Jump") {
                    MentionSender.shared.select(channel: Channel(id: message.channelID, type: .normal, guild_id: message.guildID ?? message.reference?.guildID, position: nil, parent_id: nil))
