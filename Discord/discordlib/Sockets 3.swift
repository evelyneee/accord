                    ...
                    case .string(let text):
                        ...
                        if let data = text.data(using: String.Encoding.utf8) {
                            do {
                                ...
                                retDict = ((tempretDict["d"] as? [String:Any] ?? [:])[item] as? [[String:Any]]) ?? []
                                print(retDict[safe: 0]?.keys)
                                print(tempretDict.keys)
                                print((tempretDict["d"] as? [String:Any] ?? [:]).keys)
                                print((tempretDict["d"] as? [String:Any] ?? [:])["_read_state"])
